...
int ct, hor[maxn] ; 
char g[maxx][maxx] ; 
map<pair<pii,pii>, int> who ; //qual vertice q ta entre l1 e l2 ou c1 c2
map<int, pair<pii,pii>> adjs ; //entre quais linhas ou colunas o vertice i ta 
vector<int> grafo[maxn] ; 

void imp(int id){
    pii l = adjs[id].ff, c = adjs[id].ss ; 
    if(l.ff == l.ss){
        cout << "horizontal " << l.ff << " " << c.ff << " " << c.ss << "\n" ; 
    }
    else{
        cout << "vertical " << c.ff << " " << l.ff << " " << l.ss << "\n" ; 
    }
}
