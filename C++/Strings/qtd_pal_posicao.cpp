int resp[maxn];
//qual maior palindromo que termina naquela posicao?
...
vector<int> manacher(string s){
    ...
    for(int i = 0 ; i < s.size() ; i++){
        int par = (dpar[i]-1)/2, imp = dimp[i]/2 ; 
        resp[i+par] = max(resp[i+par], dpar[i]) ; 
        resp[i+imp] = max(resp[i+imp], dimp[i]) ;     
    }
